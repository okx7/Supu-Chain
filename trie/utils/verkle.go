...
type PointCache struct {
	lru      lru.BasicLRU[string, *cachedPoint]
	lock     sync.RWMutex
	ttl      time.Duration           // 缓存项的生存时间
	cleanTick *time.Ticker           // 定期清理的计时器
	quit      chan struct{}          // 停止清理协程的信号
}

// cachedPoint包含一个点及其过期时间
type cachedPoint struct {
	point    *verkle.Point  // 点数据
	expireAt time.Time      // 过期时间
}

// NewPointCache returns the cache with specified size.
func NewPointCache(maxItems int) *PointCache {
	// 默认使用1小时的TTL和5分钟的清理间隔
	return NewPointCacheWithTTL(maxItems, 1*time.Hour, 5*time.Minute)
}

// NewPointCacheWithTTL创建一个带有TTL机制的点缓存
func NewPointCacheWithTTL(maxItems int, ttl, cleanInterval time.Duration) *PointCache {
	c := &PointCache{
		lru:      lru.NewBasicLRU[string, *cachedPoint](maxItems),
		ttl:      ttl,
...
// cleanLoop在后台定期清理过期的缓存项
func (c *PointCache) cleanLoop() {
	for {
		select {
		case <-c.cleanTick.C:
			c.cleanExpired()
...
// cleanExpired清理所有过期的缓存项
func (c *PointCache) cleanExpired() {
	c.lock.Lock()
	defer c.lock.Unlock()
	...
	for _, key := range c.lru.Keys() {
		if val, ok := c.lru.Peek(key); ok {
			if now.After(val.expireAt) {
				c.lru.Remove(key)
				log.Debug("Removed expired point from cache", "key", key)
			}
		}
	}
}
