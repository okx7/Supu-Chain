...
type RateLimiterConfig struct {
	// 区块同步速率限制
	MaxBlocksPerSecond     int           // 每秒最大处理区块数
	MaxBlockBodiesPerQuery int           // 单次请求最大区块体数量
	MaxHeadersPerQuery     int           // 单次请求最大区块头数量
	
	// 缓存清理限制
	CacheCleaningInterval  time.Duration // 缓存清理间隔时间
	MaxCacheCleaningTime   time.Duration // 最大缓存清理时间
	
	// 系统资源限制
	MaxCPUPercentage       int           // 最大CPU使用百分比
	MaxMemoryPercentage    int           // 最大内存使用百分比
	
	// 限制是否启用
	RateLimitEnabled       bool          // 是否启用速率限制
}

// 默认速率限制配置
var DefaultRateLimiterConfig = RateLimiterConfig{
	MaxBlocksPerSecond:     50,             // 每秒最多处理50个区块
	MaxBlockBodiesPerQuery: 256,            // 单次最多请求256个区块体
	MaxHeadersPerQuery:     256,            // 单次最多请求256个区块头
	CacheCleaningInterval:  30 * time.Minute, // 每30分钟清理一次缓存
	MaxCacheCleaningTime:   1 * time.Minute,  // 单次缓存清理最多1分钟
	MaxCPUPercentage:       80,             // 最多使用80%的CPU
	MaxMemoryPercentage:    80,             // 最多使用80%的内存
	RateLimitEnabled:       true,           // 默认启用速率限制
}

// 全局速率限制配置实例
var GlobalRateLimiterConfig = DefaultRateLimiterConfig

// 资源控制相关变量
var (
	// 速率限制器
	blockRateLimiter       = time.NewTicker(time.Second)
	blockRateLimiterCount  = atomic.NewInt32(0)
	lastCacheCleaning      = time.Now()
	cacheCleaning          = atomic.NewBool(false)
)
...
type BlockChain struct {
	...
	// 添加速率限制器配置
	rateLimiterConfig *RateLimiterConfig  // 速率限制配置
}
...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, genesis *Genesis, overrides *ChainOverrides, engine consensus.Engine,
	...
	*/
	...
	bc := &BlockChain{
		...
		rateLimiterConfig:  &GlobalRateLimiterConfig, // 添加速率限制器配置
	}
...
//
// Note: This method is not concurrent-safe with respect to chain mutations.
// While InsertChain may be called concurrently if the chain being inserted
// doesn't touch the main chain, it should not be called concurrently with
// other chain mutations (e.g. SetHead, SetFinalized, etc).
func (bc *BlockChain) InsertChain(chain types.Blocks) (int, error) {
	// 如果启用了速率限制
	if bc.rateLimiterConfig.RateLimitEnabled {
		// 检查插入的区块数量是否超过限制
		if len(chain) > bc.rateLimiterConfig.MaxBlocksPerSecond {
			log.Warn("区块插入速率超过限制，进行限流", 
				"请求数量", len(chain), 
				"限制", bc.rateLimiterConfig.MaxBlocksPerSecond)
			
			// 只处理配置允许的最大数量
			chain = chain[:bc.rateLimiterConfig.MaxBlocksPerSecond]
		}
		
		// 检查当前秒内已处理的区块数
		currentCount := blockRateLimiterCount.Load()
		if currentCount >= int32(bc.rateLimiterConfig.MaxBlocksPerSecond) {
			// 如果已达到每秒处理上限，等待下一秒
			log.Warn("区块处理达到每秒上限，等待下一秒继续", 
				"当前已处理", currentCount, 
				"每秒上限", bc.rateLimiterConfig.MaxBlocksPerSecond)
			
			// 简单等待至下一个时间片
			<-blockRateLimiter.C
			blockRateLimiterCount.Store(0)
		}
		
		// 更新本次将要处理的区块数
		blockRateLimiterCount.Add(int32(len(chain)))
	}
...
// 添加速率限制器刷新函数
func (bc *BlockChain) rateLimiterRefresher() {
	...
	for {
		...
		case <-ticker.C:
			...
			if bc.rateLimiterConfig.RateLimitEnabled && 
			   !cacheCleaning.Load() && 
			   time.Since(lastCacheCleaning) > bc.rateLimiterConfig.CacheCleaningInterval {
				go bc.controlledCacheCleaning()
			}
...
// 受控的缓存清理，防止资源耗尽
func (bc *BlockChain) controlledCacheCleaning() {
	// 确保同一时间只有一个清理进程
	if !cacheCleaning.CompareAndSwap(false, true) {
		return
	}
	
	defer func() {
		cacheCleaning.Store(false)
		lastCacheCleaning = time.Now()
	}()
	
	// 创建一个定时器，限制清理时间
	cleaningTimeout := time.After(bc.rateLimiterConfig.MaxCacheCleaningTime)
	
	// 进行实际的缓存清理工作
	cleanStart := time.Now()
	log.Info("开始受控缓存清理", "最大时间", bc.rateLimiterConfig.MaxCacheCleaningTime)
	
	// 这里是清理缓存的实际逻辑
	// 例如清理区块缓存、状态缓存等
	bc.stateCache.TrieDB().Cap(bc.cacheConfig.TrieCleanLimit)
	
	select {
	case <-cleaningTimeout:
		log.Warn("缓存清理达到最大允许时间，强制停止", "用时", time.Since(cleanStart))
	default:
		log.Info("缓存清理完成", "用时", time.Since(cleanStart))
	}
}
