...
// ValidateMessageFormat 对消息进行严格的格式校验，防止恶意数据包攻击
func ValidateMessageFormat(msg Msg) error {
	// 检查消息大小是否合理
	if msg.Size > 10*1024*1024 {
		return errors.New("消息大小超过限制")
	}
	
	// 检查消息码是否在有效范围内
	if msg.Code > 0xFFFF {
		return errors.New("消息码超出有效范围")
	}
	
	// 检查是否存在消息负载
	if msg.Payload == nil {
		return errors.New("消息负载为空")
	}
...
func (msg Msg) Decode(val interface{}) error {
	...
	if err := ValidateMessageFormat(msg); err != nil {
		return newPeerError(errInvalidMsg, "消息格式校验失败: %v", err)
	}
